...
function TrainingPlans({ data, loading, onAdd, onEdit, onFocus, onDelete, onStatus }: { data: TrainingData; loading: boolean; onAdd: () => void; onEdit: (training: Training) => void; onFocus: (training: Training) => void; onDelete: (id: number) => Promise<boolean>; onStatus: (id: number, status: TrainingStatus) => Promise<boolean> }) {
  ...
  const monthGroups = groupTrainingsByMonth(data.trainings);
  const cardActions = { data, onEdit, onFocus, onDelete, onStatus };

  return <div className="space-y-4">
    <Accordion key={monthGroups[0]?.key ?? "no-training-months"} type="multiple" defaultValue={monthGroups[0] ? [monthGroups[0].key] : []} className="space-y-3">
...
function monthKey(date: Date) {
  return date.toLocaleDateString("sv-SE", { timeZone: "Europe/Oslo" }).slice(0, 7);
}
...
function groupTrainingsByMonth(trainings: Training[]) {
  ...
  return [...groups.entries()]
    .map(([key, entries]) => ({ key, trainings: entries.sort((a, b) => {
      if (a.status !== b.status) return (a.status === "planned" ? 0 : a.status === "completed" ? 1 : 2) - (b.status === "planned" ? 0 : b.status === "completed" ? 1 : 2);
      if (a.status === "planned" && (a.date >= today) !== (b.date >= today)) return a.date >= today ? -1 : 1;
      const chronological = `${a.date}|${a.startTime}`.localeCompare(`${b.date}|${b.startTime}`);
      return a.status === "planned" ? chronological : -chronological;
    }) }))
    .sort((a, b) => {
      const aUpcoming = a.trainings.some((training) => training.status === "planned" && training.date >= today);
      const bUpcoming = b.trainings.some((training) => training.status === "planned" && training.date >= today);
