...
declare
  ...
  mid bigint := nullif(payload->>'id','')::bigint;
...
begin
  ...
  if a='saveCup' then
    if btrim(coalesce(payload->>'name',''))='' or nullif(payload->>'startDate','') is null or nullif(payload->>'endDate','') is null then
      raise exception 'Navn og dato må fylles ut.';
    end if;
    ...
    if team_count > current_count then
      for current_count in current_count+1..team_count loop
        insert into public.cup_teams(cup_id,name,sort_order)
        values(cid,case when team_count=1 then 'STHK' else 'STHK '||current_count end,current_count);
      end loop;
    elsif team_count < current_count then
      if exists(select 1 from public.matches m join public.cup_teams t on t.id=m.cup_team_id where t.cup_id=cid and t.sort_order>team_count) then
        raise exception 'Fjern kampene til laget før du reduserer antall lag.';
      end if;
      delete from public.cup_teams where cup_id=cid and sort_order>team_count;
    end if;
    if team_count>1 then
      update public.cup_teams set name='STHK '||sort_order where cup_id=cid;
    else
      update public.cup_teams set name='STHK' where cup_id=cid;
    end if;

    update public.matches m
    set period_count=c.period_count,
        period_minutes=c.period_minutes,
        competition=c.name,
        cup_name=c.name,
        team=ct.name
    from public.cups c
    join public.cup_teams ct on ct.cup_id=c.id
    where m.cup_id=cid and c.id=cid and m.cup_team_id=ct.id and m.status='planned';
    ...
    if mid is null then
      ...
      update public.matches set
        ...
        start_time=btrim(coalesce(payload->>'startTime','')),
        opponent=btrim(payload->>'opponent'),
        team=team_row.name,
        home_away=case when payload->>'homeAway'='away' then 'away' else 'home' end,
        competition=cup_row.name,
        match_type='cup',
        cup_name=cup_row.name,
        venue=coalesce(nullif(btrim(coalesce(payload->>'venue','')),''),cup_row.venue),
        period_count=cup_row.period_count,
        period_minutes=cup_row.period_minutes,
        notes=btrim(coalesce(payload->>'notes','')),
        cup_id=cid,
        cup_team_id=cup_team_key
      where id=mid and cup_id=cid and status='planned';
      if not found then raise exception 'Bare planlagte cupkamper kan redigeres.'; end if;
...
-- Bring existing planned cup matches in line with the cup settings.
update public.matches m
set period_count=c.period_count,
    period_minutes=c.period_minutes
from public.cups c
where m.cup_id=c.id
  ...
  and (m.period_count is distinct from c.period_count or m.period_minutes is distinct from c.period_minutes);
