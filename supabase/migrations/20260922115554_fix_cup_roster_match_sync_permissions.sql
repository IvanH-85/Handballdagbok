-- Keep planned match rosters in sync inside the already authorized admin RPC.
-- The previous row trigger ran with the signed-in user's table permissions and
-- therefore could not write to match_players, whose direct access is revoked.
drop trigger if exists sync_cup_player_to_planned_matches on public.cup_team_players;
drop function if exists app_private.sync_cup_player_to_planned_matches();
...
begin
  ...
  select distinct m.id, ctp.player_id, false, false, false, 'bench'
  from public.matches m
  join public.cup_team_players ctp
    on ctp.cup_id = m.cup_id
