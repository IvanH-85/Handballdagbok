alter table public.cups
  ...
  check (period_count between 1 and 2);
...
begin
  ...
  if a='saveCup' then
    ...
    if registrar_id is not null and not exists(
      ...
      join public.cup_team_players ctp on ctp.player_id=pg.player_id and ctp.cup_id=cid and ctp.cup_team_id=ctid
    ...
    values(
      ...
      cup_row.period_count,cup_row.period_minutes,btrim(coalesce(payload->>'notes','')),cid,ctid
...
begin
  return jsonb_build_object(
    'cups',(select coalesce(jsonb_agg(jsonb_build_object(
      'id',id,'name',name,'startDate',start_date,'endDate',end_date,
      'venue',venue,'periodCount',period_count,'periodMinutes',period_minutes,'createdAt',created_at
    ) order by start_date,name),'[]') from public.cups),
    'cupTeams',(select coalesce(jsonb_agg(jsonb_build_object(
      'id',id,'cupId',cup_id,'name',name,'sortOrder',sort_order
    ) order by cup_id,sort_order,id),'[]') from public.cup_teams),
    'cupTeamPlayers',(select coalesce(jsonb_agg(jsonb_build_object(
      'cupId',cup_id,'cupTeamId',cup_team_id,'playerId',player_id
    ) order by cup_id,cup_team_id,player_id),'[]') from public.cup_team_players),
    'playerGuardians',case when u.role='admin' then
      (select coalesce(jsonb_agg(jsonb_build_object('playerId',player_id,'userId',user_id) order by player_id,user_id),'[]') from public.player_guardians)
      else '[]'::jsonb end,
    'cupMatchLinks',(select coalesce(jsonb_agg(jsonb_build_object(
      'matchId',id,'cupId',cup_id,'cupTeamId',cup_team_id
    ) order by cup_id,date,start_time,id),'[]') from public.matches where cup_id is not null)
  );
end;
$function$
